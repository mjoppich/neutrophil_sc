...
print(unique(obj.integrated.patient_thr$orig.ident))
write_cell_barcodes(obj.integrated.patient_thr, "/mnt/input/own/SeqMappings/Pekayvaz/21053_thrombus/used_cellbarcodes/", c("sepsis1", "sepsis2"))
...
saveRDS(de.sepsis.control, "../de.patint_thr.control.sepsis.Rds")
saveRDS(de.sepsis.thrombus, "../de.patint_thr.thrombus.sepsis.Rds")
saveRDS(de.thrombus.control, "../de.patint_thr.control.thrombus.Rds")
saveRDS(rownames(obj.integrated.patient_thr), "../de.patint_thr.universe.Rds")

# ~/Rscript.sh /mnt/input/own/SeqMappings/Pekayvaz/scripts/enrichmentAnalysis.R ../de.patint_thr.control.sepsis.Rds ../de.patint_thr.universe.Rds human
# ~/Rscript.sh /mnt/input/own/SeqMappings/Pekayvaz/scripts/enrichmentAnalysis.R ../de.patint_thr.thrombus.sepsis.Rds ../de.patint_thr.universe.Rds human
# ~/Rscript.sh /mnt/input/own/SeqMappings/Pekayvaz/scripts/enrichmentAnalysis.R ../de.patint_thr.control.thrombus.Rds ../de.patint_thr.universe.Rds human

# ~/Rscript.sh /mnt/input/own/SeqMappings/Pekayvaz/scripts/enrichmentAnalysis.R ../de.granulocyte_results.Rds ../obj_granulocyte_universe.Rds human
# ~/Rscript.sh /mnt/input/own/SeqMappings/Pekayvaz/scripts/enrichmentAnalysis.R ../de.granulocyte_subclustering_results.Rds ../obj_granulocyte_universe.Rds human
...
save_plot = function(plotobj,outname, fig.width, fig.height)
{
  ...
  if (class(plotobj$data) %in% c("list"))
  {
    ...
    for (i in 1:length(plotobj$data))
    {
      ...
      print("Data is no list")
      fname = paste(outname,"data", sep=".")
      print(paste("Saving to file", fname))

      df = apply(plotobj$data,2,as.character)
      write.table(df, paste(outname, "data", sep="."), row.names = TRUE, sep="\t")
  }
...
names(enrichment.granulocyte)

dir.create("enrichment")

p=DimPlot(obj.granulocyte, pt.size = 0.001, label=T, group.by="idents")
save_plot(p, "enrichment/dimplot_subset_granulocyte", fig.width=12, fig.height=8)

makeEnrichmentPlots(enrichment.control.sepsis, "patint_thr_enrichment/control_sepsis/")
makeEnrichmentPlots(enrichment.thrombus.sepsis, "patint_thr_enrichment/thrombus_sepsis/")
makeEnrichmentPlots(enrichment.control.thrombus, "patint_thr_enrichment/control_thrombus/")


makeEnrichmentPlots(enrichment.granulocyte, "patint_thr_enrichment/patint_thr_granulocytes_sub/")
makeEnrichmentPlots(enrichment.granulocyte_subclustering, "patint_thr_enrichment/patint_thr_granulocytes_subclustering/")


makeCombinedDotPlot = function(enrichResult)
{
  ...
  for (comparison in names(enrichResult))
  {
    ...
    for (perf_test in all_tests)
    {
        ...
        result = filter(result_orig, Count > 0)
        result = filter(result, qvalue < 0.2)
        ...
        if (!(perf_test %in% names(takenDotPlots)))
        {
          takenDotPlots[[perf_test]] = list()
          takenBarPlots[[perf_test]] = list()
        }

        term2length = lapply(result@geneSets, function(x){length(x)})

        p=dotplot(result, x="Count", color="p.adjust", size="GeneRatio", showCategory=30) + ggtitle(paste("dotplot for", perf_test))
        p$data$termsize = term2length[rownames(p$data)]
        takenDotPlots[[perf_test]][[comparison]] = c(takenDotPlots[[perf_test]][[comparison]], p)

        p=mutate(result, qscore = -log(qvalue, base=10)) %>% barplot(x="qscore", fill="qvalue", showCategory=30)
        p$data$termsize = term2length[rownames(p$data)]
        takenBarPlots[[perf_test]][[comparison]] = c(takenBarPlots[[perf_test]][[comparison]], p)
    }
  }

  return(list("dot"=takenDotPlots, "bar"=takenBarPlots))
}


combplots = makeCombinedDotPlot(enrichment.granulocyte)

gooPlots = combplots$dot$goo
gooPlotsExt = list()
for (pname in names(gooPlots))
{
  gooPlotsExt[[pname]] = gooPlots[[pname]]$data[,c("ID", "Description", "GeneRatio", "Count", "p.adjust", "termsize")]
  gooPlotsExt[[pname]]$plotname = pname
}
combinedDF = Reduce(function(x, y) rbind(x, y), gooPlotsExt)  
head(combinedDF)

combinedDF$plotname = factor(combinedDF$plotname, levels=unique(combinedDF$plotname))
combinedDF$plotname_n = as.numeric(combinedDF$plotname)

combinedDF$plot_description = paste(combinedDF$Description, " (n=", combinedDF$termsize, ")", sep="")
combinedDF$plot_description = factor(combinedDF$plot_description, levels=unique(combinedDF$plot_description))
combinedDF$desc_n = as.numeric(combinedDF$plot_description)

combinedDF$percn = 1
col.min=0
col.max=max(combinedDF$p.adjust)
cols = c("red", "blue")

plotElem_orig <- ggplot(combinedDF) +
  scale_x_continuous(breaks=combinedDF$plotname_n, labels=combinedDF$plotname) +
  scale_y_continuous(breaks=combinedDF$desc_n, labels=function(x) {stringr::str_wrap(stringr::str_replace_all(combinedDF$plot_description, "#+#" , " "),width = 40)}, limits = c(min(combinedDF$desc_n)-0.6, max(combinedDF$desc_n)+0.6))+
  scale_fill_distiller(palette='Spectral', limits = c(0,1))+
  scale_size_continuous(range = c(0, 10), limits=c(0,1))+
  geom_point(aes(x=plotname_n, y=desc_n, colour = p.adjust, size = GeneRatio*10)) +
  scale_color_gradient(limits=c(col.min, col.max), low = cols[1], high = cols[2])+
  guides(color=guide_colourbar(title="P. Adjust", order = 1),
  size=guide_legend(title="GeneRatio", order = 2))+
   theme(axis.title.x=element_blank(), axis.title.y=element_blank(), axis.text.x = element_text(angle = 45, hjust=1),  legend.position = "bottom", panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line())

save_plot(plotElem_orig, "test2", fig.width=10, fig.height=10)


makeEnrichmentPlots = function(enrichResult, outfolder)
{

  dir.create(paste(outfolder, "enrichment", sep="/"))

  for (comparison in names(enrichResult))
  {

    dir.create(paste(outfolder, "enrichment", comparison, sep="/"))
    ...
    for (perf_test in all_tests)
    {
        ...
        dir.create(paste(outfolder,"enrichment", comparison, perf_test, sep="/"))
        ...
        write_xlsx(result_orig@result, paste(outfolder,"enrichment", comparison, perf_test, "enrichment_result.xlsx", sep="/"))

        p=dotplot(result, showCategory=30) + ggtitle(paste("dotplot for", perf_test))
        ...
        cnetResult = filter(result, Count > 1)

        if (nrow(cnetResult) > 0)
        {
        p <- cnetplot(cnetResult, foldChange = testExpression)
        save_plot(p, paste(outfolder,"enrichment", comparison, perf_test, "cnetplot", sep="/"), fig.width=12, fig.height=12)
        }
        ...
        p <- treeplot(result2, nCluster=ncluster)
        save_plot(p, paste(outfolder,"enrichment", comparison, perf_test, "treeplot", sep="/"), fig.width=8, fig.height=12)

        p <- emapplot(result2)
        save_plot(p, paste(outfolder,"enrichment", comparison, perf_test, "emapplot", sep="/"), fig.width=8, fig.height=12)
...
cluster.granulocyte = c(0,2,3,6,8,10,12)
cells.granulocyte = cellIDForClusters(obj.integrated.patientwnn, "idents", cluster.granulocyte)

cells.sepsis = cellIDForClusters(obj.integrated.patientwnn, "condition", c("sepsis"))
cells.control = cellIDForClusters(obj.integrated.patientwnn, "condition", c("control"))

granulocyteMarkers = compareClusters(scdata=obj.integrated.patientwnn,
                            cellsID1=intersect(cells.granulocyte, cells.sepsis),
                            cellsID2=intersect(cells.granulocyte, cells.control),
                            prefix= "granulocytes",
                            suffix1="sepsis",
                            suffix2="control",
                            test="MAST", fcCutoff="0.25", assay="RNA", outfolder="patint_wnn/de")


HighlightedDimPlot = function(obj.in, highlightedClusters, group.by="idents", reduction=reduction)
{

p = DimPlot(obj.in, group.by="idents", reduction=reduction)
g <- ggplot_build(p)
...
p = DimPlot(obj.in, group.by="idents", reduction=reduction, cols=colorList)
return(p)

}

p=HighlightedDimPlot(obj.integrated.patientwnn, highlightedClusters=cluster.granulocyte, reduction="wnn.umap")
save_plot(p, paste("patint_wnn", "dimplot_granulocytes", sep="/"), 8, 6)

p = DimPlot(obj.integrated.patientwnn, group.by="idents", reduction="wnn.umap")


makeDiffAnalysis(subset(obj.integrated.patientwnn, idents %in% cluster.granulocyte), "patint_wnn_granulocytes", 'wnn.umap')


de.control.sepsis = compareCellsByCluster(obj.integrated.patientwnn, cells.control, cells.sepsis, "control", "sepsis", outfolder=paste("patint_wnn", "de", sep="/"))
makeVolcanos(de.control.sepsis, "DE Control vs Sepsis", paste("patint_wnn", "de_volcano", "ctrl_sepsis_volcano", sep="/"), turnExpression=F, FCcutoff=0.25, pCutoff = 0.05)
...
##
###
#### Neutro sub (patint_wnn)
###
##
#


obj.granulocyte = subset(obj.integrated.patientwnn, cells=cells.granulocyte)

p=DimPlot(obj.granulocyte, pt.size = 0.001, label=T)
save_plot(p, "patint_wnn_granulocytes_sub/dimplot_granulocyte", fig.width=12, fig.height=8)

#obj.granulocyte <- RunPCA(obj.granulocyte, npcs = 50, reduction.name="igpca", assay="integrated_gex")
#obj.granulocyte <- RunPCA(obj.granulocyte, features = rownames(obj.granulocyte[['ABA']]), verbose = FALSE, approx=FALSE, npcs=10, reduction.name="iapca",  assay="integrated_adt")
#obj.granulocyte.gex_adt <- FindMultiModalNeighbors(
#  obj.granulocyte, reduction.list = list("igpca", "iapca"), 
#  dims.list = list(1:50, 1:2), modality.weight.name = "RNA.weight", prune.SNN=1/20
#)
#
# multi modal viz
#
#obj.granulocyte.gex_adt <- RunUMAP(obj.granulocyte.gex_adt, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
#obj.granulocyte.gex_adt <- FindClusters(obj.granulocyte.gex_adt, graph.name = "wsnn", algorithm = 3, resolution = 1, verbose = FALSE)
#p=DimPlot(obj.granulocyte.gex_adt, pt.size = 0.001, label=T, reduction="wnn.umap")
#save_plot(p, "patint_wnn_granulocytes_sub/dimplot_subset_granulocyte", fig.width=12, fig.height=8)
#p=DimPlot(obj.granulocyte.gex_adt, split.by="condition", label=T, reduction="wnn.umap")
#save_plot(p, "patint_wnn_granulocytes_sub/dimplot_subset_granulocyte_bycondition", fig.width=14, fig.height=8)
...
obj.granulocyte.gex_adt <- obj.granulocyte

obj.granulocyte.gex_adt$idents = Idents(obj.granulocyte.gex_adt)

p=DimPlot(obj.granulocyte.gex_adt, pt.size = 0.001, label=T)
save_plot(p, "patint_wnn_granulocytes_sub/dimplot_subset_granulocyte", fig.width=12, fig.height=8)

p=DimPlot(obj.granulocyte.gex_adt, split.by="condition", label=T)
save_plot(p, "patint_wnn_granulocytes_sub/dimplot_subset_granulocyte_bycondition", fig.width=14, fig.height=8)


p=DimPlot(obj.granulocyte.gex_adt, split.by="patient", label=T, ncol=2)
save_plot(p, "patint_wnn_granulocytes_sub/dimplot_subset_granulocyte_bypatient", fig.width=14, fig.height=32)


DefaultAssay(obj.granulocyte.gex_adt) = "RNA"


SelectDimPlot = function(obj.in, selectedClusters, group.by="condition", color.bg="#808080", color.match="#4F1E7A", reduction="umap")
{
...
colorGroupDF = unique(g$data[[1]][,c("colour", "group")])
colorGroupDF$group = as.numeric(colorGroupDF$group)-1
print(head(colorGroupDF))

uniqueGroups = unique(obj.neutros[[group.by]])[[group.by]]
selClusterIdc = which(uniqueGroups == selectedClusters)-1
print(selClusterIdc)
print(uniqueGroups)

colorGroupDF[!(colorGroupDF$group %in% selClusterIdc),]$colour = color.bg
colorGroupDF[(colorGroupDF$group %in% selClusterIdc),]$colour = color.match

colorList = as.list(colorGroupDF$colour)
names(colorList) = uniqueGroups
...
makeDiffAnalysis(obj.granulocyte.gex_adt, "patint_wnn_granulocytes_sub")

DefaultAssay(obj.granulocyte.gex_adt) = "RNA"

write_rds(obj.granulocyte.gex_adt, "../patint_wnn_granulocytes_sub.neutros.integrated.rds", compress="none")
...
DefaultAssay(obj.integrated.library) = "RNA"

origSample = unique(obj.integrated.library$orig.ident)[1]

print(paste(patientSample, origSample))

# create a new assay to store ADT information
abAssayMat1 <- allABs.raw[["sepsis1"]]
colnames(abAssayMat1) = paste("sepsis1", colnames(abAssayMat1), sep="_")
abAssayMat2 <- allABs.raw[["sepsis2"]]
colnames(abAssayMat2) = paste("sepsis2", colnames(abAssayMat2), sep="_")

abAssayMat = cbind(abAssayMat1, abAssayMat2)

gexnames = colnames(obj.integrated.library)
print(head(gexnames))

cellsJoint = intersect(gexnames, colnames(abAssayMat))

print(length(cellsJoint))
abAssayMat = abAssayMat[,cellsJoint]


abAssayMat = abAssayMat[grep("HAB", rownames(abAssayMat), invert=T, value=T), ]
#rownames(abAssayMat) = str_replace(rownames(abAssayMat), "\\.1", "")

abAssayMat2 = abAssayMat[names(rownametransform), ]
rownames(abAssayMat2) = as.character(rownametransform)

abAssay=CreateAssayObject(counts = as.matrix(abAssayMat2))

# add this assay to the previously created Seurat object
obj.integrated.library[["ABA"]] <- abAssay
obj.integrated.library <- NormalizeData(obj.integrated.library, normalization.method = "CLR", margin = 2, assay = "ABA")
...
# ADT+RNA integration: https://github.com/satijalab/seurat/issues/3890

# ADT normalization for integration: https://github.com/satijalab/seurat/issues/5154 ; CLR normalize per library!

# WNN multiple ATAC snRNA scRNA: https://github.com/satijalab/seurat/issues/5290

#My envisioned plan is as follows:
#With multi-omics as the center, RNA and ATAC are respectively integrated in sequence (scRNA+snRNA; scATAC+snATAC), and then the wnn method is used to integrate RNA and ATAC.

# WNN find clusters: https://github.com/satijalab/seurat/issues/3645
